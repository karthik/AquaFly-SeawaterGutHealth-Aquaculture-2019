...
# Import data
df <- read.csv("./data/clean_data/AqFl2_qPCR_target_PI.csv", 
               header = T, 
               na.strings = c(""),
               encoding = "UTF-8", 
...
df$Target <- factor(df$Target, levels = c("myd88", "il1β", "il8", "cd3γδ", "cd8β", "mhcI", "ifnγ","il17a", 
                                          "foxp3", "il10", "tgfβ1", "il4", "plin2", "mtp", "apoa1", "apoa4", 
                                          "apob", "chk", "pcyt1a", "fabp2b", "aqp8ab", "cldn15", "cldn25b", 
                                          "zo1", "cdh1", "muc2",  "cyp1a1", "mta", "hsp70", "sod1", "cat", 
                                          "casp6", "pcna", "mmp13")) 
...
ggsave('./results/exploratory_analysis/qPCR_target_PI_violin.pdf', 
       units = "in", 
       dpi = 300, 
...
ggsave('./results/exploratory_analysis/qPCR_target_PI_boxPlot.pdf', 
       units = "in", 
       dpi = 300, 
       encoding = "CP1253", 
       height = 24, 
...
annCol = HeatmapAnnotation(df = df_w[ , 2:3], 
                           col = list(Diet = c("REF" = "red", "IM100" = "blue"), # customize color
...
x <- c("Immune modulation", "lipid metabolism", "barrier function", "xenobiotic matebolism")
times <- c(12, 8, 6, 8)
annRow <- data.frame(func = rep(x, times = times))
annRow$func <- factor(annRow$func, levels = c("Immune modulation",
                                              "lipid metabolism",
                                              "barrier function",
                                              "xenobiotic matebolism"))

pdf('./results/exploratory_analysis/qPCR_target_PI__heatmap.pdf', 
    encoding = "CP1253", 
    height = 10,
    width = 8.5)
...
pdf(file = "./results/statistics/qPCR_target_PI_resid_plot.pdf", encoding = "CP1253") 
...
pdf(file = "./results/statistics/qPCR_target_PI_qqplot.pdf", encoding = "CP1253")
...
# Get session info 
writeLines(capture.output(sessionInfo()), "./code/qPCR_target_PI_sessionInfo.txt")
