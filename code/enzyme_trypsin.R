...
df <- read.csv("./data/AqFl2_trypsin.csv", header = T, na.strings = c(""))
df
...
df %>% 
  ...
  summarize(N = n(), Mean = mean(Trypsin_activity), SD = sd(Trypsin_activity), SE = SD / N^(1/2)) %>%
  ggplot(aes(x = Gut_segment, y = Mean, fill = Diet)) +
    ...
    scale_y_continuous(limits = c(0, 500), breaks = 0:10*50, expand = c(0, 0)) +
    theme_bw() +
    theme_classic()+
    labs(title = "Trypsin activity", x = "Gut segment", y = "U/mg DM") +
...
ggsave("trypsin.tiff", units = "in", dpi = 300, compression = "lzw", path = "./results/figures")
...
t_test <- lapply(df_spl, function(x) t.test(Trypsin_activity ~ Diet, data = x))
