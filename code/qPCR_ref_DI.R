...
ggsave('./results/exploratory_analysis/qPCR_ref_DI_pointDiagram.pdf', 
       units = "in", 
       dpi = 300,
       height = 10,
       width = 8)
...
ggsave('./results/exploratory_analysis/qPCR_ref_DI_boxPlot.pdf', units = "in", dpi = 300)
...
ggsave('./results/exploratory_analysis/qPCR_ref_DI_barPlot.pdf', units = "in", dpi = 300)
...
pdf('./results/statistics/qPCR_ref_evaluation_DI.pdf', width = 12)
gridExtra::grid.table(smr)
dev.off()

# Get session info
writeLines(capture.output(sessionInfo()), "./code/qPCR_ref_DI_sessionInfo.txt")